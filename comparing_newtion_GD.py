...
def convergence_curve(f, grad_f, hess_f, x_star, x0_factor, name, max_iter = 250):
    ...
    plt.tight_layout()
    plt.savefig(f"examples/comparing_newton_GD/convergence_curve_{name.lower()}.jpg", bbox_inches = "tight")
    plt.show()
...
# unpack history 
history_newton = np.array(result_newton.history)    # shape (nit, 2)
xs_newton, ys_newton = history_newton[:, 0], history_newton[:, 1]

history_GD = np.array(result_GD.history)    # shape (nit, 2)
...
Z = 100*(Y - X**2)**2 + (1 - X)**2
plt.contour(X, Y, Z, levels=np.logspace(-1, 3, 30), cmap="gray", alpha=0.3, linewidths = 0.5)


# plot histories
plt.plot(xs_newton, ys_newton, "o-", color="tab:blue", markersize=1, linewidth=1.5, label="Newton")
plt.plot(xs_GD, ys_GD, "o-", color="tab:red", markersize=1, linewidth=1.5, label="Gradient Descent")

plt.scatter(x0[0], x0[1], marker = "*", color = "green", label = "Start", s = 100)
plt.scatter(rosenbrock_x_star[0], rosenbrock_x_star[1], marker = "*", color = "tab:purple", label = "Minimum", s = 100, zorder = 1)

# plt.xlim(x0[0]-1, rosenbrock_x_star[0]+1)
# plt.ylim(x0[1]-1, rosenbrock_x_star[1]+1)
plt.title("Rosenbrock Optimization path")
plt.xlabel("x")
plt.ylabel("y")
plt.legend()
plt.savefig("comparing_newton_GD/rosenbrock_trajectory_paths.jpg")
...
plt.tight_layout()
plt.savefig(r"C:\Users\kripa\OneDrive - University of Ottawa\Desktop\numoptlib\examples\comparing_newton_GD/himmelblau_trajectory_paths.jpg", bbox_inches = "tight")
plt.show()
...
ani.save(r"C:\Users\kripa\OneDrive - University of Ottawa\Desktop\numoptlib\examples/t_path.gif", writer='pillow')