...
def test_quadratic_converges(quadratic):
    ...
    x0 = np.zeros(2)
    result = adams_gradient_descent(f, grad_f, x0, alpha=0.1, max_iter=2000)
...
def test_rosenbrock_converges(rosenbrock):
    f, grad_f = rosenbrock
    x0 = np.array([-1.0, 1.0])
    result = adams_gradient_descent(f, grad_f, x0, alpha=0.01, max_iter=5000)
    assert result.success
    np.testing.assert_allclose(result.x, np.ones(2), atol=1e-2)

def test_already_at_minimum(quadratic):
    f, grad_f, x_star = quadratic
    result = adams_gradient_descent(f, grad_f, x_star)
    assert result.nit <= 2

def test_max_iter_respected(rosenbrock):
    f, grad_f = rosenbrock
    x0 = np.array([-1.0, 1.0])
    result = adams_gradient_descent(f, grad_f, x0, max_iter=10)
...
def test_result_has_expected_fields(quadratic):
    f, grad_f, x_star = quadratic
    x0 = np.zeros(2)
    result = adams_gradient_descent(f, grad_f, x0)
...
def test_history_length_matches_nit(quadratic):
    f, grad_f, x_star = quadratic
    x0 = np.zeros(2)
    result = adams_gradient_descent(f, grad_f, x0, max_iter=50)
...
def test_default_hyperparameters_match_paper(quadratic):
    ...
    import inspect
    sig = inspect.signature(adams_gradient_descent)
    params = sig.parameters
    assert params["alpha"].default == 0.001
...
def test_higher_alpha_faster_on_quadratic(quadratic):
    ...
    f, grad_f, x_star = quadratic
    x0 = np.zeros(2)
    result_slow = adams_gradient_descent(f, grad_f, x0, alpha=0.001, max_iter=2000)
    result_fast = adams_gradient_descent(f, grad_f, x0, alpha=0.1,   max_iter=2000)
...
def test_adaptive_scaling_helps_ill_conditioned():
    ...
    x_star = np.linalg.solve(A, b)
    x0 = np.zeros(2)
    result = adams_gradient_descent(f, grad_f, x0, alpha=0.1, max_iter=5000)
    assert result.success
    np.testing.assert_allclose(result.x, x_star, atol=1e-3)
...
def test_moment_estimates_initialized_at_zero():
    ...
    grad_f = lambda x: x
    x0 = np.array([1.0, 1.0])
    result = adams_gradient_descent(f, grad_f, x0, max_iter=1)
...
def test_epsilon_prevents_division_by_zero():
    ...
    x0 = np.array([1.0, 1.0])
    result = adams_gradient_descent(f, grad_f, x0, max_iter=100)
    assert np.all(np.isfinite(result.x))

def test_no_line_search_used(quadratic, monkeypatch):
    """Adam should not call line_search at all."""
    import numoptlib.unconstrained.adams as adam_module
    called = []
    ...
    f, grad_f, x_star = quadratic
    x0 = np.zeros(2)
    adams_gradient_descent(f, grad_f, x0, max_iter=10)
    assert len(called) == 0