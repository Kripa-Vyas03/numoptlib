...
ax1.contourf(X, Y, Z, levels=np.logspace(0, 3, 30), cmap="gray", alpha=0.15)
ax1.contour(X, Y, Z, levels=np.logspace(0, 3, 30), colors="gray", alpha=0.5, linewidths=0.5)

for x0, color in zip(starts, colors):
    result = newton(himmelblau, himmelblau_grad, x0, hessian_f = himmelblau_hessian)
...
plt.tight_layout()
#plt.savefig("examples/newton/himmelblau_newton_paths.png", dpi=150, bbox_inches="tight")
plt.show()