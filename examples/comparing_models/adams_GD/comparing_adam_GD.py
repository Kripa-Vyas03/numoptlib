...
def plot_quadratic_optim(name, f, grad_f, x_star, x0 = np.array([5.0, 3.0]), alpha = 0.5, max_iter = 500):
    ...
    result_adam = adam_gradient_descent(f, grad_f, x0, alpha = alpha, max_iter = max_iter)
    
    history_gd  = np.array(result_gd.history)
    history_adam = np.array(result_adam.history)
    
    # --- contour grid ---
    margin = 0.5
    all_x = np.concatenate([history_gd[:, 0], history_adam[:, 0], [WC_quad_x_star[0]]])
    all_y = np.concatenate([history_gd[:, 1], history_adam[:, 1], [WC_quad_x_star[1]]])
    ...
    plot_path(axes[1], history_adam, color="mediumseagreen",
              label="adam path", title=f"adam  ({result_adam.nit} iters)")
    ...
    print(f"adam  — iters: {result_adam.nit}, nfev: {result_adam.nfev}, converged: {result_adam.success}")
...
def convergence_curve(f, grad_f, x_star, x0_factor, name, alpha = 0.1, max_iter = 500):
    ...
    result_GD = gradient_descent(f, grad_f, x0, max_iter = max_iter) 
    result_m = adam_gradient_descent(f, grad_f, x0, max_iter = max_iter, alpha = alpha)   
    ...
    if name.lower() == "rosenbrock":
        axs[0].plot(range(result_m.nit + 1), m_gap, color = "tab:blue", label = "adam")
        axs[1].plot(range(result_m.nit + 1), grad_norm_m, color = "tab:blue", label = "adam")
        
    else:
        axs[0].plot(range(result_m.nit), m_gap, color = "tab:blue", label = "adam")
        axs[1].plot(range(result_m.nit), grad_norm_m, color = "tab:blue", label = "adam")
    ...
    plt.tight_layout()
    plt.savefig(f"convergence_curve_{name.lower()}_adam_GD.jpg", bbox_inches = "tight")
    plt.show()
...
for x0 in ([[0, 0], [-1.1, 1], [-1.5, 1.3]]):
    x0 = np.array(x0)
    result_m = adam_gradient_descent(rosenbrock, rosenbrock_grad, x0, max_iter = 2000, alpha = 0.1)
    ...
    ax1.plot(history_m[:, 0], history_m[:, 1], "o-", color="tab:red",  markersize=0.5, linewidth=1.5, label = f"adam, {result_m.nit} iterations")
    ...
    plt.tight_layout()
    plt.savefig(f"rosenbrock_adam_GD_start{x0}.png", dpi=150, bbox_inches="tight")
    plt.show()
...
for x0, x_star, cols in zip(starts, himmelblau_x_star, colors):
    ...
    result_m = adam_gradient_descent(himmelblau, himmelblau_grad, x0, max_iter = 2000, alpha = 0.1)
    history_m = np.array(result_m.history)
    loss_values_m = [rosenbrock(x) for x in result_m.history]
    ...
    plt.plot(history_m[:, 0], history_m[:, 1], color=mcolor, linewidth=1, label=f"adam, {result_m.nit} iterations")
...
plt.savefig("himmelblau_adam_GD.png", bbox_inches = "tight")
plt.show()
