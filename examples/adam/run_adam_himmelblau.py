...
for x0, color in zip(starts, colors):
    result = adam_gradient_descent(himmelblau, himmelblau_grad, x0, alpha = 0.1, max_iter = 2000)
    ...
    ax1.plot(history[:, 0], history[:, 1], "o-", color=color,
             markersize=2, linewidth=0.5, label=f"Start {x0}, {result.nit} iterations")
...
plt.tight_layout()
plt.savefig("himmelblau_adam_paths.png", dpi=150, bbox_inches="tight")
plt.show()