...
for x0 in ([[-1, 1], [0, 0], [-1.1, 1]]):
    ...
    plt.tight_layout()
    plt.savefig(f"rosenbrock_gradient_descent_path_start{x0}.png", dpi=150, bbox_inches="tight")
    plt.show()
...
result1 = momentum_gradient_descent(rosenbrock, rosenbrock_grad, x0, beta = 0.99, max_iter = 2000)
result2 = momentum_gradient_descent(rosenbrock, rosenbrock_grad, x0, beta = 0.5, max_iter = 2000)
result3 = momentum_gradient_descent(rosenbrock, rosenbrock_grad, x0, beta = 0.1, max_iter = 2000)
...
ax1.set_title("Optimization path with varying decay rates")
ax1.set_xlabel("x")
ax1.set_ylabel("y")
...
for r, beta, c in zip([result1, result2, result3], [0.99, 0.5, 0.01], ['tab:blue', "tab:red", "tab:orange"]):
    ...
    ax1.plot(history[:, 0], history[:, 1], "o-", color = c, markersize=0.5, linewidth=1.5)
    
    loss_values = [rosenbrock(x) for x in r.history]
    ax2.plot(loss_values, color=c, linewidth=1.5, label = fr"$\beta$ = {beta}, {r.nit} iterations")
